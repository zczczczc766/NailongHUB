...
if not B then
    pcall(function()
        A:SetCore("SendNotification", {
            Title = "WindUI加载失败",
            Text = "请检查执行器的网络/HttpGet支持",
            Duration = 5
        })
    end)
    warn("[被遗弃] WindUI加载失败:", winduiLastError)
    return
end
...
-- 兼容原脚本的 Window / Tab / Section API，让下面的功能代码尽量不动。
local activeSection = nil
local createdElements = {}

local function rememberElement(element, title)
    if element then
        table.insert(createdElements, {Element = element, Flag = "OldUI_" .. tostring(#createdElements + 1), Title = title})
    end
    return element
end
...
local function makeTabAdapter(tab)
    ...
    function adapter:Button(cfg)
        cfg = cfg or {}
        local c = {}
        for k,v in pairs(cfg) do c[k] = v end
        c.Title = c.Name or c.Title or "按钮"
        c.Name = nil
        return rememberElement(tab:Button(c), c.Title)
    end

    function adapter:Toggle(cfg)
        cfg = cfg or {}
        local c = {}
        for k,v in pairs(cfg) do c[k] = v end
        c.Title = c.Name or c.Title or "开关"
        c.Name = nil
        return rememberElement(tab:Toggle(c), c.Title)
    end

    function adapter:Slider(cfg)
        cfg = cfg or {}
        local c = {}
        for k,v in pairs(cfg) do c[k] = v end
        c.Title = c.Name or c.Title or "滑块"
        c.Name = nil
        if c.Rounding and not c.Step then
            c.Step = 10 ^ (-c.Rounding)
        end
        c.Rounding = nil
        return rememberElement(tab:Slider(c), c.Title)
    end

    function adapter:Dropdown(cfg)
        cfg = cfg or {}
        local c = {}
        for k,v in pairs(cfg) do c[k] = v end
        c.Title = c.Name or c.Title or "下拉选择"
        c.Name = nil
        return rememberElement(tab:Dropdown(c), c.Title)
    end

    function adapter:Input(cfg)
        cfg = cfg or {}
        local c = {}
        for k,v in pairs(cfg) do c[k] = v end
        c.Title = c.Name or c.Title or "输入"
        c.Name = nil
        return rememberElement(tab:Input(c), c.Title)
    end

    function adapter:Textbox(cfg)
        cfg = cfg or {}
        local c = {}
        for k,v in pairs(cfg) do c[k] = v end
        c.Title = c.Name or c.Title or "输入"
        c.Name = nil
        c.Type = c.Type or "Input"
        return rememberElement(tab:Input(c), c.Title)
    end

    function adapter:Keybind(cfg)
        cfg = cfg or {}
        local c = {}
        for k,v in pairs(cfg) do c[k] = v end
        c.Title = c.Name or c.Title or "按键绑定"
        c.Name = nil
        if typeof(c.Default) == "EnumItem" then
            c.Value = c.Default.Name
        elseif c.Default then
            c.Value = tostring(c.Default):gsub("Enum.KeyCode.", "")
        end
        c.Default = nil
        local oldCallback = c.Callback
        c.Callback = function(v)
            local key = v
            if typeof(v) == "string" and Enum.KeyCode[v] then key = Enum.KeyCode[v] end
            if oldCallback then oldCallback(key) end
        end
        return rememberElement(tab:Keybind(c), c.Title)
    end

    function adapter:Paragraph(cfg)
        cfg = cfg or {}
        return tab:Paragraph({
            Title = cfg.Name or cfg.Title or "",
            Desc = cfg.Content or cfg.Desc or "",
            Image = cfg.Image,
    ...
    function adapter:Social(cfg)
        cfg = cfg or {}
        local title = cfg.Name or "信息"
        local desc = (cfg.SubName or "") .. ((cfg.SmlName and cfg.SmlName ~= "") and ("\n" .. cfg.SmlName) or "")
        pcall(function()
            tab:Paragraph({
                Title = title,
                Desc = desc,
        ...
        if cfg.copy and cfg.copy ~= "" and cfg.copy ~= "无" then
            pcall(function()
                tab:Button({
                    Title = cfg.Cbn or "复制",
                    Callback = function()
                        pcall(function() setclipboard(tostring(cfg.copy)) end)
                    end,
                })
            end)
        end
    ...
    function adapter:Video(cfg)
        cfg = cfg or {}
        pcall(function()
            tab:Paragraph({
                Title = cfg.Name or "视频",
                Desc = "视频资源：" .. tostring(cfg.Video or ""),
...
Window.RootFolder = "WindUI"
Window.ConfigFolder = "WindUI/被遗弃/config"
Window.ConfigManager = C.ConfigManager
...
function Window:Tab(name, icon)
    ...
    local finalIcon = iconMap[name] or "circle"
    local tab = activeSection:Tab({Title = name, Icon = finalIcon})
    return makeTabAdapter(tab)
end

function Window:SetKeybind(key)
    pcall(function() C:SetToggleKey(key) end)
end

function Window:Notification(title, text, notificationType, duration)
    pcall(function()
        B:Notify({
            Title = title,
            Content = text,
...
-- 保留原脚本对 library 的调用接口；配置功能改为 WindUI ConfigManager 兼容实现。
local library = B
library.SetCustomCursor = library.SetCustomCursor or function() end
library.SaveConfig = function(_, name)
    local ok, config = pcall(function() return C.ConfigManager:CreateConfig(name) end)
    if not ok or not config then return false end
    for _, item in ipairs(createdElements) do
        pcall(function() config:Register(item.Flag, item.Element) end)
    end
    return pcall(function() config:Save() end)
end
library.LoadConfig = function(_, path)
    local name = tostring(path or ""):gsub("\\", "/"):match("([^/]+)%.json$")
    ...
    local loaded = pcall(function() config:Load() end)
    return loaded
end
library.SetTheme = function(_, theme)
...
local FengYu = Window:Tab("体力区", "84411268070942")
